...
int main()
{
   ...
   for(int i=1;i<=n;i++)
    {f>>grade[i].first;
    grade[i].second=i;
    if(grade[i].first>n-1) {cout<<"Nu exista graf "; return 0;}
    sum+=grade[i].first;
    }
    if(sum%2!=0) {cout<<"Nu exista graf"; return 0;}
    for(int i=1;i<n;i++)
    for(int j=i+1;j<=n;j++) if(grade[i].first<grade[j].first) swap(grade[i],grade[j]);
    ...
    while(grade[i].first>0)
    {
        ...
        {
            grade[i+j].first--;
            if(grade[i+j].first<0){cout<<"Nu exista graf "; return 0;}
